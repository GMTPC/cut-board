                                    ...
                                    <table style="width:100%;" class="table-output">
                                        <tr>
                                            <td colspan="4" class="underlineheader" style="background-color:{{ $colordate }} ;"> 
                                                <div class="col-6-md">
                                                    <u style="color:black;"><h2 style="text-align: left; color:black;" class="text-left"><b >{{ substr($tag->brd_lot,6,4) }}</b></h2></u>
                                                </div>
                                                <div class="col-6-md">
                                                    <u style="color:black;"><h2 style="text-align: right; color:black;" class="text-right"><b >TAG WIP รอติดFOIL</b></h2></u>
                                                </div> </br>
                                            </td>
                                        </tr>
                                        ...
                                        <tr>
                                            ...
                                            {{-- // ส่วนบาร์โค้ต ยาว --}}
                                            @if ($tag->brd_amount < 10)
                                                <td colspan="4"><center>{!! DNS1D::getBarcodeHTML("B".substr($tag->ww_line,1,1).$tag->bl_code."-".$tag->pe_type_code.$tag->brd_lot.'00'.$tag->brd_amount,'C128',1,35)  !!}</center>
                                                    <small>{!! "B".substr($tag->ww_line,1,2).$tag->bl_code."-".$tag->pe_type_code." : ".$sizearr[substr($tag->pe_type_code,2,2)]." : ".$tag->ww_line."++++++++".'00'.$tag->brd_amount !!}</small>
                                            @elseif ($tag->brd_amount < 100)
                                                <td colspan="4"><center>{!! DNS1D::getBarcodeHTML("B".substr($tag->ww_line,1,1).$tag->bl_code."-".$tag->pe_type_code.$tag->brd_lot.'0'.$tag->brd_amount,'C128',1,35)  !!}</center>
                                                    <small>{!! "B".substr($tag->ww_line,1,2).$tag->bl_code."-".$tag->pe_type_code." : ".$sizearr[substr($tag->pe_type_code,2,2)]." : ".$tag->ww_line."++++++++".'0'.$tag->brd_amount !!}</small>
                                            @else
                                                <td colspan="4"><center>{!! DNS1D::getBarcodeHTML("B".substr($tag->ww_line,1,1).$tag->bl_code."-".$tag->pe_type_code.$tag->brd_lot.$tag->brd_amount,'C128',1,35)  !!}</center>
                                                    <small>{!! "B".substr($tag->ww_line,1,2).$tag->bl_code."-".$tag->pe_type_code." : ".$sizearr[substr($tag->pe_type_code,2,2)]." : ".$tag->ww_line."++++++++" !!}</small>
                                            @endif
